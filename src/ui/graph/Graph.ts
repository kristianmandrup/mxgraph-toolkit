...
export class Graph {
  ...
  constructor(container, model, renderHint, stylesheet, themes, standalone) {
    new GraphConstructor().construct(container, model, renderHint, stylesheet, themes, standalone)
  }
   ...
   */
  static foreignObjectWarningLink = 'https://desk.draw.io/support/solutions/articles/16000042487'
  ...
  updateAlternateBounds(cell, geo, willCollapse) {
    ...
    mxGraph.prototype.updateAlternateBounds.apply(this, [cell, geo, willCollapse])
  }
  ...
  foldCells(collapse, recurse, cells, checkFoldable, evt) {
    ...
    if (cells != null) {
      ...
      try {
        mxGraph.prototype.foldCells.apply(this, [collapse, recurse, cells, checkFoldable, evt])
  ...
  encodeCells(cells) {
    ...
    const cellMapping = this.createCellMapping(cloneMap, this.createCellLookup(cells, null), null)
  ...
  moveCells(cells, dx, dy, clone, target, evt, mapping) {
    ...
    var result = graphMoveCells.apply(this, [cells, dx, dy, clone, target, evt, mapping])

    if (clone) {
      const cellMapping = this.createCellMapping(mapping, this.createCellLookup(cells, null), null)
